	...
	for m=1:length(Xs)
		if n==length(Xs[m]) && isapprox(c,Cs[m]; atol=2.0)
			...
			if any(X .< 0.0) || any(X .> 300.0)
				...
				plot(1:totalcyclelength, (n-1)*50 .+ pltX, ls=lss, ms=mss, color=col, alpha=0.3, label="")
				plot(1:n, (n-1)*50 .+ X, ls=lss, ms=mss, color=col, marker=".", alpha=1.0, label="$(n)-cycle")
				
				tight_layout()
				savefig("./spacedcyclediag.pdf", bbox_inches="tight", pad_inches=0.0, dpi=300)
